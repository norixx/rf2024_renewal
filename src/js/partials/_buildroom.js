...
class RFBuildroomSlide {
  ...
  /**
   * 
   * @param {string} key - ギャラリーの種類
   * @returns {object} - スライドデータ
   */
  #getSlideData(key) {
  ...
  async #setInitPhotos() {
    ...
    try {
      ...
      if(loadError) {
        console.log('初期写真取得に失敗。nophotoに差し替えます')
        ...
        // 画像ロードエラーフラグを設定
        // this.#isLoaded = false
        this.#isPhotosLoaded[firstKey] = false
      }
  ...
  #sortDataAndSave(data) {
    // 建物メイン写真(固定表示用)がある場合はグローバル変数から配列先頭に保存
    if(typeof RF_firstbuild_photo !== 'undefined') {
      this.#photos.build.unshift(RF_firstbuild_photo)
    }
    data.forEach((elm) => {
